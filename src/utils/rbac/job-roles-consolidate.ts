...
export type ResultObject = InputObject & { activities: string[]; associatedActivities?: ActivityObject[] };
...
export function consolidateActivities(obj: InputObject): any {
    const result: any = {};
...
export type SearchKey = "Job Role Code" | "Staff Sub Group Code" | "Staff Group Code";

// Search Job Roles
export function searchRolesByKey(key: SearchKey, value: string): InputObject[] {
    if (!key || !value) {
...
export type ResultWithActivities = ResultObject & { associatedActivities: ActivityObject[] };

export function findAssociatedActivities(consolidatedObj: Partial<ResultWithActivities>, activities: ActivityObject[]): ResultWithActivities {
